...
public class SyntaxAnanlyzer {
    ...
    public SyntaxAnanlyzer() {
        try {
            output = new BufferedWriter(new FileWriter("/home/yancychan/IdeaProjects/CompilePrinciple/src/GrammarAnalysis/output.txt"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void getText() {
        ...
        try {
            bufferedReader = new BufferedReader(new FileReader("/home/yancychan/IdeaProjects/CompilePrinciple/src/GrammarAnalysis/input.txt"));
            String lString;
    ...
    public boolean analysis() {
        while (true) {
            ...
            if (string2.trim().equals("".trim()))
                ...
                display();
                // 从输入带里弹出第一个字符，并把该字符送symbolStack,同时向StateStack压入取得的状态数
                list1.pollFirst();
                stack2.push(n);
                stack.push(c);
                ...
                // 取产生式右部的字符个数（在SymbolStack和StateStack中弹出来的个数）
                int n2 = production.getNumOfP(n);
                for (int i1 = 0; i1 < n2; i1++) {
                    stack.pop();
                    stack2.pop();
                }
                // 获取产生式左边的非终结符，压入SymbolStack
                char _char1 = production.getProduction(n).charAt(0);
                stack.push(_char1);
                // 查找goto字表，找到该终结符和当前状态对应的编号，压入StateStack
                String s1 = table[stack2.peek()][getOrder(_char1)];
                if (s1.trim().equals(""))
                    return false;
                else
